// --- SHARED UTILITIES & DB (Accessible by both app.js and admin.js) ---
const DB_KEY = 'fisterSMS_db_v12_final_dynamic';

const getInitialDB = () => ({
    servers: [
        { 
            id: 1672531200001, 
            serverName: '5sim Demo Server', 
            serverCountryCode: 'ru', 
            apiKey: 'eyJhbGciOiJSUzUxMiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3ODUwNTA0MzcsImlhdCI6MTc1MzUxNDQzNywicmF5IjoiMTAzOTdkMTg3ZDlmYjdiZDIzOThkMzE4NmNlYzkxMjkiLCJzdWIiOjMzODkyMzF9.sf8puXNnxhYIOpQluafHny0zfaZ6cObAdPv21L8fY7lsuFThPvfRomj1lqCvGqkhAVXea4kjSY4a57llXNJ72XcdY_xW265B4j01kAeFL83uxcQ9t6s8Q2B-moqCbzQ68k9rcQPo2ZDfYUFf6Zm5s94kVLbrQsdJxMWP1qOMlsU9kxWIg6gVmn97jWNZxzBHpd_Vx5rOg7QdxUN_7iZjZxXs1ZfCpjQu3Ev7M6lppVDr10TI26VAjpafzkpjRkQvQj70MZWH_LVcZSu8XE0szBHLQL_j7iZ6VHwxkVDAs6b3N_lJlU-WCQWafC-NXJoATrIX7hhlnG4pfneYoKvOMA', 
    ...
    services: [
        { id: 1672531200003, serverId: 1672531200001, serviceName: 'Telegram', servicePrice: "20", serviceCode: 'telegram', country: 'russia', operator: 'any' }
    ],
...
const saveData = () => {
    db.services.forEach(s => {
        if(s.serverId) s.serverId = parseInt(s.serverId);
    });
    localStorage.setItem(DB_KEY, JSON.stringify(db));
    if (frontend && frontend.isInitialized) frontend.populateServers();
    if (adminPanel && adminPanel.isInitialized) adminPanel.updateAnalyticsDisplay();
};
...
const showMessage = (msg, type = 'success') => { const el = document.createElement('div'); el.className = `wallet-notification ${type}`; el.textContent = msg; document.body.appendChild(el); setTimeout(() => { el.style.opacity = '1'; el.style.transform = 'translateX(-50%) translateY(0)'; }, 10); setTimeout(() => { el.style.opacity = '0'; el.style.transform = 'translateX(-50%) translateY(20px)'; setTimeout(() => el.remove(), 500); }, 3000); };
const applySeoSettings = () => { document.title = sanitizeHTML(db.seo.websiteTitle); let meta = document.querySelector('meta[name="description"]'); if (!meta) { meta = document.createElement('meta'); meta.name = 'description'; document.head.appendChild(meta); } meta.content = sanitizeHTML(db.seo.websiteDescription); };
const createRipple = (event) => { const button = event.currentTarget; const circle = document.createElement("span"); const diameter = Math.max(button.clientWidth, button.clientHeight); const radius = diameter / 2; circle.style.width = circle.style.height = `${diameter}px`; circle.style.left = `${event.clientX - (button.getBoundingClientRect().left + radius)}px`; circle.style.top = `${event.clientY - (button.getBoundingClientRect().top + radius)}px`; circle.classList.add("ripple"); const ripple = button.getElementsByClassName("ripple")[0]; if (ripple) { ripple.remove(); } button.appendChild(circle); };
...
const frontend = {
    ...
    attachOriginalEventListeners: function() {
        ...
        const checkAndAttach = () => {
            if (!doc.querySelector('.login-container')) {
            ...
            doc.querySelectorAll('.form-toggle-buttons button, .form-link-text a').forEach(el => el.addEventListener('click', (e) => { e.preventDefault(); fe.switchAuthForm(el.id === 'register-toggle-btn' || e.target.textContent.includes('Sign Up') ? 'register' : 'login', e); }));
            doc.querySelectorAll('.eye-icon').forEach(icon => icon.addEventListener('click', () => fe.togglePassword(icon)));
            doc.querySelector('#menu-toggle').addEventListener('click', e => { e.stopPropagation(); doc.querySelector('#sidebar').classList.add('open'); doc.querySelector('#overlay').classList.add('open'); });
            doc.querySelector('#overlay').addEventListener('click', () => { doc.querySelector('#sidebar').classList.remove('open'); doc.querySelector('#overlay').classList.remove('open'); });
            doc.querySelectorAll('.nav-item').forEach(item => item.addEventListener('click', e => { e.preventDefault(); fe.showPage(item.dataset.page); }));
            doc.querySelectorAll('#home-page .action-button').forEach(btn => btn.addEventListener('click', function() { if (this.dataset.page) fe.showPage(this.dataset.page); }));
            ...
            doc.querySelectorAll('.recharge-option').forEach(option => option.addEventListener('click', function() {
                const method = this.dataset.method; const amountStr = prompt(`Enter amount for ${method}:`, "100");
                if (amountStr) { const amount = parseFloat(amountStr); if (!isNaN(amount) && amount > 0) { fe.wallet.balance += amount; fe.wallet.lifetimeRecharge += amount; db.analytics.todaysPayment += amount; saveData(); fe.updateDisplay(); showMessage(`Recharged ₹${amount.toFixed(2)}`, 'success'); }}
            }));
        }
        checkAndAttach();
    },
    ...
    populateServers: function() { 
        const select = frontendView.querySelector('#server-select'); 
        select.innerHTML = ''; 
        db.servers.forEach(s => select.insertAdjacentHTML('beforeend', `<option value="${s.id}" data-country="${s.serverCountryCode}">${sanitizeHTML(s.serverName)}</option>`)); 
        if (db.servers.length > 0) select.dispatchEvent(new Event('change')); 
    },

    populateServices: function(serverId) { 
        const select = frontendView.querySelector('#service-select'); 
        select.innerHTML = ''; 
        ...
        frontendView.querySelector('#server-flag-img').src = `https://flagcdn.com/${countryCode}.svg`; 
        if (services.length === 0) { select.innerHTML = `<option disabled>No services available</option>`; return; } 
        services.forEach(s => select.insertAdjacentHTML('beforeend', `<option value="${s.serviceCode}" data-price="${s.servicePrice}" data-country="${s.country}" data-operator="${s.operator}">${sanitizeHTML(s.serviceName)} - ₹${s.servicePrice}</option>`)); 
    },

    handleBuySubmit: async function(e) {
        e.preventDefault();
        const serviceSelect = frontendView.querySelector('#service-select'); 
        if (!serviceSelect.options || serviceSelect.selectedIndex < 0) { showMessage('Please select a service.', 'error'); return; } 
        ...
        if (this.wallet.balance < price) { showMessage('Insufficient balance!', 'error'); return; }

        const serverId = parseInt(frontendView.querySelector('#server-select').value);
        const server = db.servers.find(s => s.id === serverId);
        const service = {
            code: selectedServiceOption.value,
            country: selectedServiceOption.dataset.country,
            operator: selectedServiceOption.dataset.operator
        };

        if (!server || !server.apiGetNumberUrl || !server.apiKey) { showMessage('Server is not configured correctly for API calls!', 'error'); return; }
        
        showMessage('Requesting number...', 'info');
        try {
            const url = `${server.apiGetNumberUrl}/${service.country}/${service.operator}/${service.code}`;
            const response = await fetch(url, { headers: { 'Authorization': `Bearer ${server.apiKey}` } });
            const data = await response.json();
            if (response.ok && data.id && data.phone) {
                this.wallet.balance -= price; this.wallet.numbersPurchased++; db.analytics.todaysOrders++; saveData(); this.updateDisplay();
                showMessage(`Number acquired: ${data.phone}`, 'success');
                this.createActiveNumberCard(selectedServiceOption, data);
            } else { 
                const errorMsg = typeof data === 'string' ? data : (data.message || 'Could not purchase number.');
                throw new Error(errorMsg);
            }
        } catch (error) { console.error("Buy Error:", error); showMessage(`Error: ${error.message}`, 'error'); }
    },
    ...
    startTimer: function(cardElement) { 
        ...
        if (!server || !server.apiGetMessageUrl || !server.apiKey) {
        ...
        const otpCheckInterval = setInterval(async () => {
            try {
                ...
                if (data.status === 'RECEIVED' && data.sms && data.sms.length > 0) {
                    ...
                    clearInterval(otpCheckInterval); clearInterval(timerInterval);
                } else if (data.status === 'CANCELED' || data.status === 'TIMEOUT') {
                     clearInterval(otpCheckInterval); clearInterval(timerInterval);
                     statusText.textContent = "Order Expired/Canceled";
                }
            } catch (error) { console.error("OTP Check Error:", error); }
        }, 10000); // Check every 10 seconds

        const timerInterval = setInterval(() => {
            let minutes = parseInt(duration / 60, 10); let seconds = parseInt(duration % 60, 10);
            ...
            if (--duration < 0) { 
                ...
                if (cardElement.dataset.otpRevealed === "false") { 
                    ...
                    this.cancelNumber(cardElement, true); // Auto-cancel if time expires
                }
            }
        }, 1000);
        cardElement.dataset.timerIntervalId = timerInterval; cardElement.dataset.otpIntervalId = otpCheckInterval;
    },
    ...
    cancelNumber: async function(cardElement, isAutoCancel = false) {
        if (cardElement.dataset.otpRevealed === "true") { if(!isAutoCancel) showMessage('Cannot cancel, OTP was received.', 'error'); return; }
        const orderId = cardElement.dataset.orderId; const serverId = parseInt(cardElement.dataset.serverId);
        const server = db.servers.find(s => s.id === serverId);
        ...
        } catch(error) { if(!isAutoCancel) showMessage(error.message, 'error'); } 
        finally {
            clearInterval(cardElement.dataset.timerIntervalId); 
            clearInterval(cardElement.dataset.otpIntervalId);
            cardElement.remove(); 
            const doc = document.getElementById('frontend-view');
            if (doc.querySelector('#active-numbers-container').childElementCount === 0) doc.querySelector('#no-active-number-info').style.display = 'block';
        }
    },
    ...
    banNumber: async function(cardElement) {
        ...
        if (!server || !server.apiBanUrl) {
            showMessage('Ban functionality is not configured for this server.', 'error');
            cardElement.remove(); // Remove the card even if ban fails
            return;
        }
        ...
        try {
            ...
            clearInterval(cardElement.dataset.timerIntervalId); clearInterval(cardElement.dataset.otpIntervalId);
            cardElement.remove(); 
            const doc = document.getElementById('frontend-view');
            if (doc.querySelector('#active-numbers-container').childElementCount === 0) doc.querySelector('#no-active-number-info').style.display = 'block';
        }
